...
function uploadmodelimage() {
	...
	$('#fileToUpload1').on('change', function() {
		$.ajaxFileUpload({
			url : 'clothmodel/uploadclothimage',
			type : 'post',
			data : {
				"type" : enName
			},
			...
			success : function(data) {
				//图片名称
				$("#clothmodelimagename").val(data.name);
				//模型图片url
				$("#clothmodelimageurl").val(data.clothmodelimageurl);
				//模型图片md5
				$("#clothmodelimagemd5").val(data.clothmodelimagemd5);
			},
