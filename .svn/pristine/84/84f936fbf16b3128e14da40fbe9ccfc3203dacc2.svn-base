...
// 上传文件
function uploadfabricfile() {
	// 选择文件之后执行上传
	$('#fileToUpload2').on('change', function() {
		$.ajaxFileUpload({
			url : 'clothfabric/uploadfile',
			type : 'post',
			data : {
				"type" : enName
			},
			...
			success : function(data) {
				//名称
				$("#clothfabricfilename").val(data.clothfabricfilename);
				//url
				$("#clothfabricfileurl").val(data.clothfabricfileurl);
				//md5
				$("#clothfabricfilemd5").val(data.clothfabricfilemd5);
			},
