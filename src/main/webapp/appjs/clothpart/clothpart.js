...
function add(enName) {
	...
	layer.open({
		...
		content : '../clothpart/addclothpart.jsp?enName=' + enName
	});
}

//保存服装部位
function save() {
    layer.confirm("确定新增服装部位？", function() {
		var id = null;
...
//		}
		$.ajax({
			...
			url : "clothpart/addClothPart",
			type : "post",
			dataType : "json",
			data : {
				"clothpartcnname" : $("#clothpartcnname").val(),
				"clothpartengname" : $("#clothpartengname").val(),
				"clothpartstyle" : $("#clothpartstyle").val(),
				"clothpartoccasion" : $("#clothpartoccasion").val(),
				"clothpartclass" : $("#clothpartclass").val(),
				"clothpartcode" : $("#clothpartcode").val(),
			},
			async : false,
					...
					if (data.success) {
						...
						if (templateList.length > 0) {
							...
							for (var i = 0; i < templateList.length; i++) {
								...
								var dssid = template.clothpartid;
								...
								tabhtml += "<tr>"
										...
										+ "'/></td>"
										+ "<td>"
										+ template.clothpartcnname
										+ "</td>"
										+ "<td>"
										+ template.clothpartengname
										+ "</td>"
										+ "<td>"
										+ template.clothpartstyle
										+ "</td>"
										+ "<td>"
										+ template.clothpartoccasion
										+ "</td>"
										+ "<td>"
										+ template.clothpartclass
										+ "</td>"
										+ "<td>"
										+ template.clothpartcode
										+ "</td>"
										+ "<td>"
...
function update(id) {
	layer.open({
		...
		title : "修改服装部位",
		shadeClose : false, // 点击遮罩关闭
		content : '../clothpart/updateclothpart.jsp?id=' + id
	});
}
